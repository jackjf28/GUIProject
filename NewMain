    ...
    public void start(Stage primaryStage) {
        try {
            ...
            while(i != bracket.numChallengers*2) {
                ...
                if (i == bracket.numChallengers) {
                    ArrayList<Challenger> thirdContenders = new ArrayList<Challenger>();
                    thirdContenders.add(bracket.getChallenges()[bracket.numChallenges-3].getLoser());
                    thirdContenders.add(bracket.getChallenges()[bracket.numChallenges-2].getLoser());
                    bracket.getThirdPlace(thirdContenders.get(0),thirdContenders.get(1));
                }
